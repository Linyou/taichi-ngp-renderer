...
class NGP_fw:
    def __init__(self, scale, cascades, grid_size, base_res, log2_T, res, level, exp_step_factor):
        ...
        self.hid1 = ti.field(data_type, shape=(self.max_samples_shape, 64))
        self.hid2 = ti.field(data_type, shape=(self.max_samples_shape, 64))
    ...
    def sigma_layer(self):
        ...
        for sn in ti.ndrange(self.model_launch[None]):
            ...
            for i in range(64):
                ...
                self.hid1[sn, i] = data_type(ti.max(temp, 0.0)) # relu
            
            for i in range(16):
                temp = init_val[0]
                for j in ti.static(range(64)):
                    temp += self.hid1[sn, j] * self.sigma_weights[64*32+i*64+j]
                self.hid2[sn, i] = temp

            self.out_1[self.temp_hit[sn]] = data_type(ti.exp(self.hid2[sn, 0]))
            for i in ti.static(range(16)):
                self.final_embedding[sn, i] = self.hid2[sn, i]
    ...
    def rgb_layer(self):
        ...
        for sn in ti.ndrange(self.model_launch[None]):
            ...
            for i in range(64):
                ...
                self.hid1[sn, i] = data_type(ti.max(0.0, temp))

            for i in range(64):
                temp = init_val[0]
                for j in ti.static(range(64)):
                    temp += self.hid1[sn, j] * self.rgb_weights[64*32+i*64+j]

                self.hid2[sn, i] = data_type(ti.max(0.0, temp))

            for i in ti.static(range(3)):
                temp = init_val[0]
                for j in ti.static(range(64)):
                    temp += self.hid2[sn, j] * self.rgb_weights[64*32+64*64+i*64+j]

                self.hid1[sn, i] = temp

            for i in ti.static(range(3)):
                self.out_3[self.temp_hit[sn], i] = data_type(1 / (1 + ti.exp(-self.hid1[sn, i])))
